...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div>
                <form method="POST" action="{{ route('pse.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    <x-ui.card :title="__('Formulir Pendataan')">
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <x-form.fieldset label="{{ __('Nama Sistem Elektronik') }}">
                                    <x-form.text-input name="system_name" type="text" :value="old('system_name')" autofocus />
                                ...
                                <x-form.fieldset label="{{ __('Sektor') }}">
                                    <x-form.select name="sector">
                                        ...
                                        @foreach ($sectors as $key => $label)
                                            <option value="{{ $key }}" @selected(old('sector') === $key)>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </x-form.select>
                            ...
                            <x-form.fieldset label="{{ __('Daftar Subdomain') }}">
                                <x-form.chips-input name="subdomains" :value="old('subdomains') ?? []" :hasError="$errors->has('subdomains')"
                            ...
                            <x-form.fieldset label="{{ __('Surat Permohonan Subdomain (PDF)') }}">
                                ...
                                <x-form.input-description field="surat_subdomain"
                                    value="{{ __('Wajib diunggah. Satu berkas untuk seluruh subdomain di atas. Maksimal 5 MB.') }}" />
                            </x-form.fieldset>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <x-form.fieldset label="{{ __('Nama PIC') }}">
                                    <x-form.text-input name="pic_name" type="text" :value="old('pic_name')" />
                                ...
                                <x-form.fieldset label="{{ __('Nomor Telepon PIC') }}">
                                    <x-form.text-input name="pic_phone" type="tel" :value="old('pic_phone')" />
                                ...
                                <x-form.fieldset label="{{ __('Email PIC') }}">
                                    <x-form.text-input name="pic_email" type="email" :value="old('pic_email')" />
                                ...
                                <x-form.fieldset label="{{ __('Kategori Risiko') }}">
                                    <x-form.select name="risk_category">
                                        ...
                                        @foreach ($riskCategories as $key => $label)
                                            <option value="{{ $key }}" @selected(old('risk_category') === $key)>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </x-form.select>
                                ...
                                <x-form.fieldset label="{{ __('Klasifikasi Data') }}">
                                    <x-form.select name="data_classification">
                                        ...
                                        @foreach ($dataClassifications as $key => $label)
                                            <option value="{{ $key }}" @selected(old('data_classification') === $key)>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </x-form.select>
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    @foreach ($storageLocations as $key => $label)
                                        <x-form.radio name="storage_location" :value="$key" :label="$label"
                                            :checked="old('storage_location') === $key" />
                                    @endforeach
                                </div>
                        ...
                        <x-ui.card :title="__('Formulir Pendataan')">
                            <div class="space-y-4">
                                ...
                                <p class="text-sm text-base-content/70 italic">
                                    {{ __('Lengkapi spesifikasi sumber daya server yang dibutuhkan di Diskominfo.') }}
                                </p>
                                    ...
                                    <x-form.fieldset label="{{ __('Tipe Pengajuan') }}">
                                        <x-form.select name="hosting_request_type">
                                            @foreach ($hostingMetadata['request_types'] as $key => $label)
                                                <option value="{{ $key }}" @selected(old('hosting_request_type', 'baru') === $key)>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </x-form.select>
                                    ...
                                    <x-form.fieldset label="{{ __('Tipe Hosting') }}">
                                        <x-form.select name="hosting_type">
                                            @foreach ($hostingMetadata['hosting_types'] as $key => $label)
                                                <option value="{{ $key }}" @selected(old('hosting_type', 'vps') === $key)>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </x-form.select>
                                    ...
                                    <x-form.fieldset label="{{ __('CPU Cores') }}">
                                        <x-form.select name="cpu_cores">
                                            @foreach ($hostingMetadata['cpu_cores'] as $core)
                                                <option value="{{ $core }}" @selected(old('cpu_cores', 2) == $core)>
                                                    {{ $core }} Core
                                                </option>
                                            @endforeach
                                        </x-form.select>
                                    ...
                                    <x-form.fieldset label="{{ __('RAM Capacity') }}">
                                        <x-form.select name="ram_capacity">
                                            @foreach ($hostingMetadata['ram_capacities'] as $ram)
                                                <option value="{{ $ram }}" @selected(old('ram_capacity', 4) == $ram)>
                                                    {{ $ram }} GB
                                                </option>
                                            @endforeach
                                        </x-form.select>
                                    ...
                                    <x-form.fieldset label="{{ __('Storage Capacity') }}">
                                        <x-form.select name="storage_capacity">
                                            @foreach ($hostingMetadata['storage_capacities'] as $storage)
                                                <option value="{{ $storage }}" @selected(old('storage_capacity', 50) == $storage)>
                                                    {{ $storage }} GB
                                                </option>
                                            @endforeach
                                        </x-form.select>
                                    ...
                                    <x-form.fieldset label="{{ __('Bandwidth') }}">
                                        <x-form.select name="bandwidth_capacity">
                                            @foreach ($hostingMetadata['bandwidth_capacities'] as $bandwidth)
                                                <option value="{{ $bandwidth }}" @selected(old('bandwidth_capacity', 1000) == $bandwidth)>
                                ...
                                <x-form.fieldset label="{{ __('Surat Permohonan (PDF)') }}">
                                    <x-form.file-input id="surat_permohonan" name="surat_permohonan"
                                        accept=".pdf" />
                                    <x-form.input-error :messages="$errors->get('surat_permohonan')" />
                                    <x-form.input-description field="surat_permohonan"
                                        value="{{ __('Wajib diupload jika memilih lokasi Aplikasi. Ukuran maksimal 5 MB.') }}" />
                                </x-form.fieldset>
                                ...
                                <x-form.fieldset label="{{ __('Catatan Tambahan') }}">
                                    ...
                                    <x-form.input-description field="hosting_notes"
                                        value="{{ __('Opsional. Jelaskan kebutuhan spesifik lainnya...') }}" />
                                </x-form.fieldset>
                            </div>
