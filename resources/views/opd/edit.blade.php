...
@section('section', __('Edit Instansi'))

<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center px-4 sm:px-0">
            <x-ui.heading level="2">
                {{ __('Edit Instansi / OPD') }}
            </x-ui.heading>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-ui.card :title="__('Formulir Instansi')">
                <form action="{{ route('opd.update', $opd) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-4">
                        ...
                        <x-form.fieldset label="{{ __('Nama Instansi (OPD)') }}">
                            <x-form.text-input id="name" name="name" type="text" :value="old('name', $opd->name)" required
                                autofocus />
                        ...
                        <x-form.fieldset label="{{ __('Tipe (Opsional)') }}">
                            <x-form.text-input id="type" name="type" type="text" :value="old('type', $opd->type)" />
                        ...
                        <x-form.fieldset label="{{ __('Email Resmi (Opsional)') }}">
                            <x-form.text-input id="email" name="email" type="email" :value="old('email', $opd->email)" />
